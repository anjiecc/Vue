<template>
    <div id="uploadJs">
        ...
        <el-upload class="upload-demo" action="/file/upload" :on-preview="handlePreview" :on-remove="handleRemove" :file-list="fileList2" list-type="picture" :before-upload="beforeLoad" :on-success="uploadSuccess">
            ...
            <div slot="tip" class="el-upload__tip">只能上传text/markdown文件，列表会显示最近一天添加的文章</div>
        </el-upload>
    </div>
</template>

<script>
const typeArray = ['text/markdown'];
export default {
    data() {
        return {
            fileList2: [],
    ...
    methods: {
        ...
        beforeLoad(file) { //上传之前回调
            if (typeArray.indexOf(file.type) < 0) {
                this.$notify.error({
                    title: '错误',
                    message: '目前只支持text/markdown格式的文件!'
                });
                return false;
            }
        ...
        uploadSuccess(res, file, fileList) { // 上传成功回调
            if (res.type) {
                var _url = `/file/addFile?id='${new Date().getTime()}'&name=${file.name}&url=${res.originalUrl}&size=${res.size}&date=${new Date(new Date().getTime()).toLocaleString()}&author=${this.author.split(';')[0]}&type=${res.headers['content-type']}&fileType=md`;
...
<style>
#uploadJs input[type="file"] {
    display: none;
}

#uploadJs .upload-demo,
#addPhoto .el-upload__tip {
    text-align: left;
}

#uploadJs .el-upload.el-upload--picture {
    margin: 5px 0 0 5px;
}
</style>
